<template>
  <aside class="w-60 bg-[#1a1b1e] border-l border-white/[0.04] flex-shrink-0 overflow-y-auto custom-scrollbar h-full flex flex-col">
    <!-- Banner, a todo el ancho del panel -->
    <div class="h-28 relative flex-shrink-0" :style="{ background: bannerStyle }">
      <div class="absolute inset-0 bg-gradient-to-t from-[#1a1b1e] via-[#1a1b1e]/10 to-transparent"></div>
    </div>

    <!-- Avatar con anillo, solapando el banner -->
    <div class="px-4 -mt-11 relative z-10">
      <div class="inline-block rounded-full p-[5px] bg-[#1a1b1e]">
        <UserAvatar
    ...
    <div class="h-px bg-white/[0.06] mx-4 my-3.5"></div>

    <!-- Estado -->
    <div class="px-4 flex items-center gap-2 text-[12px] text-gray-300">
    ...
    <!-- Miembro desde -->
    <div class="px-4 flex items-center gap-2.5 text-gray-400">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
      <span class="text-[13px] text-gray-300 truncate">Miembro desde {{ memberSince }}</span>
    </div>
    ...
    <!-- Servidores en común -->
    <div class="px-2.5">
      <div v-if="isLoadingExtra" class="h-8 mx-1.5 rounded-lg bg-white/[0.03] animate-pulse"></div>
      <button
        v-else
        @click="mutualCommunities.length > 0 && (showCommunities = !showCommunities)"
        :disabled="mutualCommunities.length === 0"
        class="w-full flex items-center justify-between px-1.5 py-2 rounded-lg transition-colors disabled:cursor-default"
        :class="mutualCommunities.length > 0 ? 'hover:bg-white/[0.04]' : ''"
      >
      ...
      <div v-if="showCommunities && mutualCommunities.length > 0" class="pb-1 flex flex-col gap-1">
        <div
          v-for="c in mutualCommunities"
          :key="c.id"
          class="flex items-center gap-2.5 px-1.5 py-1.5 rounded-lg hover:bg-white/[0.04] transition-colors"
        >
          <div class="w-7 h-7 rounded-lg overflow-hidden flex-shrink-0 bg-[#141518] flex items-center justify-center">
    ...
    <div class="px-2.5">
      ...
      <button
        ...
        class="w-full flex items-center justify-between px-1.5 py-2 rounded-lg transition-colors disabled:cursor-default"
        :class="mutualFriends.length > 0 ? 'hover:bg-white/[0.04]' : ''"
      >
        <span class="text-[12px] font-semibold text-gray-300">
      ...
      <div v-if="showFriends && mutualFriends.length > 0" class="pb-1 flex flex-col gap-1">
        <button
          v-for="f in mutualFriends"
          :key="f.id"
          @click="profileStore.open(f.id)"
          class="w-full flex items-center gap-2.5 px-1.5 py-1.5 rounded-lg hover:bg-white/[0.04] transition-colors text-left"
        >
