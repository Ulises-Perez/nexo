<template>
  ...
  >
    <template #header="{ titleId }">
      <h2 :id="titleId" class="sr-only">Configuración de canal — {{ channel?.name }}</h2>
    </template>
...
const handleSave = async () => {
  ...
  if (success) {
    ...
    toast.error('Hubo un error al guardar el canal.');
  }
};
...
const handleDelete = async () => {
  ...
  const confirmed = await dialog.confirm({
    message: `¿Eliminar el canal "${props.channel.name}" definitivamente?`,
    danger: true,
  });
  if (!confirmed) return;
  ...
  if (success) {
    ...
    toast.error('Hubo un error al eliminar el canal.');
  }
};
</script>
